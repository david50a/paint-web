...
export const SignUpPage: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/'); // Redirect to home or dashboard after successful registration and login
    } catch (err: any) {
        ...
        <div className="px-10 pb-10 space-y-5">
          ...
          >
            ...
            <div className="flex items-center gap-3 py-2">
              ...
              />
              <label htmlFor="terms" className="text-[11px] text-canvas-ink/60 cursor-pointer select-none">
                I agree to the{' '}
                <button
                  type="button"
          ...
          <p className="text-center text-sm text-canvas-ink/70">
            ...
            <Link to="/signin" className="text-canvas-accent hover:underline">
              Sign In
            </Link>
          </p>
        </div>
      ...
      <AnimatePresence>
        {showPolicy && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-canvas-bg/90 backdrop-blur-xl"
          >
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
